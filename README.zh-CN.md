...
展开，有改动暂存时标题上出现标记，底部一次保存把显示货币、费率表与时段规则以一次带版本号
约束的写入一起提交。保存成功后卡片重新折叠。
...
改变的是金额读起来是什么，从来不是费率本身的含义。费率条目上出现未知字段会在加载时直接报错
而不是被忽略，因此 schema 无法兑现的值绝不会被悄悄丢弃。
...
设置卡片里有一块 **显示货币**：每种内置货币一个预设按钮，点一下就把代码、符号和汇率一起暂存，
...
- `test/host.test.mjs` —— 汇总、时段、计价、显示货币的默认值与各种拒绝，以及 schema 的
  各种拒绝。用 `node --test` 运行，无外部依赖。
- `test/client.test.mjs` —— 卡片的折叠行为（折叠、展开、暂存改动、底部单次保存、放弃、
  自动折叠）、它的货币切换，以及胶囊的计价与格式化，在 jsdom 中用 React Testing Library
  挂载。
